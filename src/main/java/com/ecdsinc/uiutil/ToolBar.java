/**
 * This class overrides the JToolBar class to fix problems with vertical
 * toolbars in the JDK 1.3 implementation.
 */
...
public class ToolBar extends JToolBar {
	...
	public static final int      DOCK_NORTH      = 0x1;
	public static final int      DOCK_EAST       = 0x2;
	public static final int      DOCK_SOUTH      = 0x4;
	public static final int      DOCK_WEST       = 0x8;

	public static final int      DOCK_ALL        = 0xf;
	public static final int      DOCK_HORIZONTAL = 0x5;
	public static final int      DOCK_VERTICAL   = 0xa;
	...
	/**
	 * Set the toolbar's dock mask.  The dock mask specifies which docking
	 * locations are valid for this tool bar.
	 *
	 * @param dockMask - specifies the valid docking locations for this toolbar
	 * can be a combination of any of the following:
	 *
	 *    	DOCK_NORTH, DOCK_EAST, DOCK_SOUTH, DOCK_WEST
	 */
	public void setDockMask(int dockMask) {
		m_DockMask = dockMask;
	}

	/**
	 * Return the toolbar's dock mask.  The dock mask specifies which docking
	 * locations are valid for this tool bar.
	 *
	 * @return the toolbar's docking mask
	 */
	public int getDockMask() {
		return m_DockMask;
	}

	public void dock (String dockPosition)
	{
		//  Does nothing if UI is not an EcdsBasicToolBarUI instance
		if (getUI() instanceof EcdsBasicToolBarUI) {
			((EcdsBasicToolBarUI) getUI()).dock(dockPosition);
		}
	}

	private void replaceUI() {

		//  Currently only replace the ToolBar UI if the current UI is the
		//  BasicToolBarUI.  Need to handle MetalToolBarUI seperatly
		if (getUI() instanceof BasicToolBarUI) {
			setUI (new EcdsBasicToolBarUI());
		}
	}

	private int     m_DockMask = DOCK_ALL;
 ...
 *
 * The implementation for this class was copied from the BasicToolBarUI class
 * in the JDK 1.3.  This class was modified from the JDK 1.3 implementation
 * to fix problems with vertical toolbars
 */
class EcdsBasicToolBarUI extends ToolBarUI implements SwingConstants
{
	...
	//  ADDED - Data member to track last docked position
	private String             m_LastDockPosition = "";

    public static ComponentUI createUI( JComponent c )
    {
	    return new EcdsBasicToolBarUI();
    }
	...
	//  ADDED - Added function dock.  This function forces the toolbar to be
	//  docked at the position specified by docPosition
	public void dock(String docPosition) {

		if ((docPosition != BorderLayout.NORTH) &&
			(docPosition != BorderLayout.SOUTH) &&
			(docPosition != BorderLayout.EAST) &&
			(docPosition != BorderLayout.WEST)) {
		    throw new IllegalArgumentException("Dock Position invalid");
		}

		m_LastDockPosition = docPosition;
		setFloating(false, null);
	}
    ...
    public void setFloating(boolean b, Point p) {
	 	if (toolBar.isFloatable() == true) {
			...
			if (b == true)
		    {
				...
				//  MODIFIED - set orientation based on toolbar orientation
				setOrientation(toolBar.getOrientation());
				floatingFrame.pack();
				...
				//  MODIFIED - redock to last position docked if available
				String constraint;
				if ((m_LastDockPosition.length() != 0) && (p == null)) {
					constraint = m_LastDockPosition;
				}
				else {
					constraint = getDockingConstraint(dockingSource, p);
					m_LastDockPosition = constraint;
				}
   ...
   public void setDockingColor(Color c) {
    ...
    public boolean canDock(Component c, Point p) {
		...
		if (c.contains(p)) {
			...
			//  MODIFIED - Set docking sensitivity based on orientation
			if (dockingSensitivity == 0) {
				if (toolBar.getOrientation() == JToolBar.HORIZONTAL) {
			        dockingSensitivity = toolBar.getSize().height;
				}
				else {
					dockingSensitivity = toolBar.getSize().width;
				}
			}

			//  MODIFIED - Added dockMask
			int dockMask = ((ToolBar)toolBar).getDockMask();

			// North
			if (p.y < dockingSensitivity) {
				//MODIFIED - Added dockMask checking
				if ((dockMask & ToolBar.DOCK_NORTH) != 0) {
			        b = true;
				}
			}
			...
			{
				//MODIFIED - Added dockMask checking
				if ((dockMask & ToolBar.DOCK_SOUTH)  != 0) {
			        b = true;
				}
			}
			...
			if (p.x < dockingSensitivity){
				//MODIFIED - Added dockMask checking
				if ((dockMask & ToolBar.DOCK_WEST) != 0) {
			        b = true;
				}
			}
			...
			if (p.x > c.getSize().width-dockingSensitivity) {
				//MODIFIED - Added dockMask checking
				if ((dockMask & ToolBar.DOCK_EAST) != 0) {
			        b = true;
				}
			}
    ...
    private String getDockingConstraint(Component c, Point p) {
		...
		if ((p != null) && (c.contains(p))) {
			...
			//  MODIFIED - set dockingSensitivity based on orientation
			if (dockingSensitivity == 0) {
				if (toolBar.getOrientation() == JToolBar.HORIZONTAL) {
				    dockingSensitivity = toolBar.getSize().height;
				}
			...
			// South  (Base distance on height for now!)
			if (p.y > c.getSize().height-dockingSensitivity) {
				//MODIFIED - Added dockMask checking
				if ((dockMask & ToolBar.DOCK_SOUTH) != 0) {
	...
	private static class RightAction extends KeyAction {
        public void actionPerformed(ActionEvent e) {
			...
			EcdsBasicToolBarUI ui = (EcdsBasicToolBarUI)toolBar.getUI();
		...
		public void setOrientation(int o) {
		    if(isShowing()) {
				...
				if (offset!=null) {
					...
					//  Implementation modified becuase
					//  BasicGraphicsUtils.isLeftToRigth has package access
